...
class KayobeAnsibleMixin(object):
    ...
    def handle_kolla_tags_limits_deprecation(self, parsed_args):
        ...
        if (parsed_args.kolla_limit or parsed_args.kolla_tags or
            ...
            LOG.warning(f"{yellow}[WARNING] The use of --kolla-tags, "
                        "--kolla-limit, and --kolla-skip-tags is deprecated. "
                        "Please switch to just using --tags, --limit, or "
                        "--skip-tags, these are now passed into kolla-ansible "
                        "too. KOlla tags/limit will be removed in the next "
                        f"release.{reset}")
        if parsed_args.limit and parsed_args.kolla_limit:
            LOG.error(f"{red}[ERROR] You can no longer use both --limit and "
                      "--kolla-limit at the same time. Please switch to just "
                      f"using --limit{reset}")
            sys.exit(1)
        if parsed_args.tags and parsed_args.kolla_tags:
            LOG.error(f"{red}[ERROR] You can no longer use both --tags and "
                      "--kolla-tags at the same time. Please switch to just "
                      f"using --tags{reset}")
            sys.exit(1)
        if parsed_args.skip_tags and parsed_args.kolla_skip_tags:
            LOG.error(f"{red}[ERROR] You can no longer use both --skip-tags "
                      "and --kolla-limit at the same time. Please switch to "
                      f"just using --limit{reset}")
            sys.exit(1)
